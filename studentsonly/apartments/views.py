...
def building(request, building_slug): #Building detail
    ...
    allReviews = Reviews.objects.filter(building=building)
    allApts = Apartment.objects.filter(building=building) 
    score = Reviews.objects.filter(building=building).aggregate(Avg('overall'))#annotate or aggrogate
...
def postreview(request):
    if request.method == 'POST':
        ...
        form = ReviewForm()
        context = {'form': form}
    return render(request, 'apartments/review.html', context)
